...
def main():
    ...
    for buyer in buyers:
        ...
        X_sub = buyer["X"][:, used_features]
        Y = buyer["Y"]
        sub_market = DataMarketplace(X_sub, Y, lambda_penalty=0.1)
        shapley_weights = sub_market.allocate_revenue(pn=p_n, bn=buyer["mu"], method='approximate', K=100)
